 ...
 * @package dompdf
 */
abstract class Frame_Decorator extends DP_Frame {
   ...
   */
  function __construct(DP_Frame $frame, DOMPDF $dompdf) {
    $this->_frame = $frame;
    $this->_root = null;
    $this->_dompdf = $dompdf;
   ...
   */
  function deep_copy() {
    $frame = new DP_Frame($this->get_node()->cloneNode());
    $frame->set_style(clone $this->_frame->get_original_style());
  ...
  /**
   * @param DP_Frame $child
   * @param bool  $update_node
   */
  function prepend_child(DP_Frame $child, $update_node = true) {
   ...
   * @param bool  $update_node
   */
  function append_child(DP_Frame $child, $update_node = true) {
    while ( $child instanceof Frame_Decorator ) {
      $child = $child->_frame;
    }
  ...
  /**
   * @param DP_Frame $new_child
   * @param DP_Frame $ref
   * @param bool  $update_node
   */
  function insert_child_before(DP_Frame $new_child, DP_Frame $ref, $update_node = true) {
   ...
   * @param bool  $update_node
   */
  function insert_child_after(DP_Frame $new_child, DP_Frame $ref, $update_node = true) {
    while ( $new_child instanceof Frame_Decorator ) {
      $new_child = $new_child->_frame;
    }
   ...
   * @return Frame
   */
  function remove_child(DP_Frame $child, $update_node = true) {
    while  ( $child instanceof Frame_Decorator ) {
      $child = $child->_frame;
    }
   ...
   * @param DP_Frame $root
   */
  function set_root(DP_Frame $root) {
    $this->_root = $root;
   ...
   * @return void
   */
  function split(DP_Frame $child = null, $force_pagebreak = false) {
